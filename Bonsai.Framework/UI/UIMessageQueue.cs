...
namespace Bonsai.Framework.UI
{
    ...
    {
        SpriteFont font;
        Queue<TextElement> msgs = new Queue<TextElement>();
        TextElement update_currentMessage;
        ...
        public void AddMessage(StringBuilder msg, Vector2 position, eMessageType type)
        {
            switch (type)
            {
                case eMessageType.FadingText_Fast:
                    msgs.Enqueue(new TextElement(msg, font, position, Color.Orange, true, true));
                    break;

                case eMessageType.FadingText_Slow:
                    msgs.Enqueue(new TextElement(msg, font, position, Color.Orange, true, false));
                    break;

                case eMessageType.StaticText:
                    msgs.Enqueue(new TextElement(msg, font, position, Color.GreenYellow, false, false));
                    break;
