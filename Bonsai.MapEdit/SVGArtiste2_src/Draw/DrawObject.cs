...
namespace Draw
{
    ...
    public abstract class DrawObject : ICloneable
    {
        ...
        public static string GetStringGameProps(bool isPassable, bool isDeath)
        {
            return $"data-IsPassable=\"{isPassable}\" data-IsDeath=\"{isDeath}\"";
        }
        ...
        public void SetGamePropsFromSvg(SvgRect svg)
        {
            var el = svg as SvgElement;
            ArrayList types = new ArrayList();
            ArrayList names = new ArrayList();
            ArrayList values = new ArrayList();
            el.FillAttributeList(types, names, values);

            for (var x = 0; x < names.Count; x++)
            {
                if (names[x].ToString() == "data-IsPassable")
                {
                    IsPassable = bool.Parse(values[x].ToString());
                }
                if (names[x].ToString() == "data-IsDeath")
                {
                    IsDeath = bool.Parse(values[x].ToString());
                }
            }
